...
func (fc *RFBConn) recvServerInit(s []byte, err error) {
	...
	glog.Debugf("serverInit:%+v, %+v", si, si.PixelFormat)
	fc.s = si
...
func (fc *RFBConn) recvRectHeader(s []byte, err error) {
	...
	glog.Debugf("rect:%+v, len=%d", rect, int(rect.Width)*int(rect.Height)*4)
...
func (fc *RFBConn) recvRectBody(s []byte, err error) {
	...
	fc.NbRect--
	glog.Debug("fc.NbRect:", fc.NbRect)
	if fc.NbRect == 0 {
